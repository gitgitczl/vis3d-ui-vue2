...
class Prompt {
    ...
    constructor(viewer, opt) {
        ...
        this.promptDiv.className = `vis3d-prompt ${this.opt.className}`;
        ...
        // 绑定面板点击事件
        if (this.opt.click) {
            let contentDom = window.document.getElementById(`prompt-content-${this.opt.id}`);
            contentDom.addEventListener('click', e => {
                that.opt.click(this.opt);
            })
        }
