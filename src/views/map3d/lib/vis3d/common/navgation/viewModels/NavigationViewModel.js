...
NavigationViewModel.prototype.show = function (container) {
  ...
  var testing =
    '<div class="compass" id="vis3d-compass" title="" style="display:' + compassDisplay + '" data-bind="visible: showCompass, event: { mousedown: handleMouseDown, dblclick: handleDoubleClick }">' +
    ...
    '</div>' +
    '<div class="navigation-controls" id="vis3d-navigation-controls" style="display:' + enableZoomControlsDisplay + '">' +
    '<!-- ko foreach: controls -->' +
...
NavigationViewModel.prototype.setStyle = function (style) {
  ...
  let ele = document.getElementById("vis3d-compass");
  let ele2 = document.getElementById("vis3d-navigation-controls");
  if (!ele) return;
  for (let i in style) {
    ele.style[i] = style[i];
