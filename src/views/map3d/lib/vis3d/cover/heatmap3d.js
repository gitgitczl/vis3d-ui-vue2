    ...
    bindTooltip() {
        ...
        this.popupHandler.setInputAction(function (evt) { //单击开始绘制
            ...
            if (!pick || !pick.primitive || pick.primitive.id != 'heatmap3d') {
                ...
                const position = that.viewer.scene.pickPosition(evt.endPosition);
                const rate = that.computeRateInBound(position);
                console.log("rate--->",rate)
                const value = that.heatmapInstance.getValueAt(rate);
                if (that.opt.moueseMove) that.opt.moueseMove({
                    value: value, px: evt.endPosition, position: position
                });
            }
