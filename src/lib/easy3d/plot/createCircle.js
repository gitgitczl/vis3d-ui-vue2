...
class CreateCircle extends BasePlot{
  ...
  start(callBack) {
    if (!this.prompt)
      this.prompt = new Prompt(viewer, { offset: { x: 30, y: 30 } });
  ...
  createByPositions(lnglatArr, callback) {
    ...
    if (Array.isArray(lnglatArr)) {
      ...
      if (isCartesian3) {
        ...
        positions = cUtil.lnglatsToCartesians(lnglatArr);
      }
      ...
      this.floatPosition = cUtil.getPositionByLength();
    }
  ...
  updatePointHeight(h) {
    ...
    centerP = cUtil.updatePositionsHeight(
      [centerP],
      Number(this.style.height)
    )[0];
    floatP = cUtil.updatePositionsHeight(
      [floatP],
      Number(this.style.height)
    )[0];
  ...
  getPositions(isWgs84) {
    let positions = [];
    if (isWgs84) {
      positions = cUtil.cartesiansToLnglats([this.position, this.floatPosition]);
    } else {
