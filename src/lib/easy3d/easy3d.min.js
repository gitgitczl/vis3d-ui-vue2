...
cUtil$1.cartesianToLnglat = function (cartesian, viewer) {
  if (!cartesian) return [];
...
cUtil$1.getViewCenter = function (viewer) {
  if (!viewer) return;
...
cUtil$1.cartesiansToLnglats = function (cartesians, viewer) {
  ...
  for (var i = 0; i < cartesians.length; i++) {
    arr.push(cUtil$1.cartesianToLnglat(cartesians[i], viewer));
  }
...
cUtil$1.lnglatsToCartesians = function (lnglats) {
...
cUtil$1.flyTo = function (opt, viewer) {
  if (!viewer) {
...
cUtil$1.getCameraView = function (viewer) {
  viewer = viewer || window.viewer;
...
cUtil$1.setCameraView = function (obj, viewer) {
  viewer = viewer || window.viewer;
...
cUtil$1.oreatationToHpr = function (position, orientation, toDegrees) {
...
cUtil$1.wgs2gcj = function (arrdata) {
...
cUtil$1.gcj2wgs = function (arrdata) {
  var lng = Number(arrdata[0]);
  var lat = Number(arrdata[1]);
  var dlat = transformWD(lng - 105.0, lat - 35.0);
  var dlng = transformJD(lng - 105.0, lat - 35.0);
...
cUtil$1.lerpPositions = function (positions, granularity) {
  if (!positions || positions.length == 0) return;
  var dis = 0;

  for (var _i = 1; _i < positions.length; _i++) {
    dis += Cesium.Cartesian3.distance(positions[_i], positions[_i - 1]);
  }
...
cUtil$1.getIntersectPosition = function (obj, viewer) {
  if (!viewer) {
...
cUtil$1.getCirclePointsByAngle = function (center, aimP, angle) {
...
cUtil$1.getCirclePointsByRadius = function (opt) {
  var _ref = opt || {},
      center = _ref.center,
      radius = _ref.radius,
      angle = _ref.angle;
...
cUtil$1.computeAngle = function (p1, p2) {
...
cUtil$1.updatePositionsHeight = function (pois, h) {
  if (!pois || h == undefined) return;
  var newPois = [];

  for (var i = 0; i < pois.length; i++) {
    var c3 = pois[i];
    var ct = cUtil$1.cartesianToLnglat(c3);
...
cUtil$1.computeUniforms = function (positions, isOn3dtiles, viewer) {
  if (!viewer) {
    console.log('cUtil.computeUniforms缺少viewer对象');
    return;
  }

  var area = cUtil$1.computeArea(positions, viewer) / 1000;
  if (!positions) return;
  ...
  for (var index = 0; index < indices.length; index = index + 3) {
    ...
    if (!isOn3dtiles) {
      h1 = cUtil$1.getTerrainHeight(cartesian1, viewer);
    } else {
      h1 = cUtil$1.get3dtilesHeight(cartesian1, viewer);
    }
    ...
    if (!isOn3dtiles) {
      h2 = cUtil$1.getTerrainHeight(cartesian2, viewer);
    } else {
      h2 = cUtil$1.get3dtilesHeight(cartesian2, viewer);
    }
    ...
    if (!isOn3dtiles) {
      h3 = cUtil$1.getTerrainHeight(cartesian3, viewer);
    } else {
      h3 = cUtil$1.get3dtilesHeight(cartesian3, viewer);
    }
    ...
    obj.area = cUtil$1.computeAreaOfTriangle(cartesian1, cartesian2, cartesian3);
...
cUtil$1.computeArea = function (positions, viewer) {
  if (!viewer) {
  ...
  var lnglats = cUtil$1.cartesiansToLnglats(positions, viewer);
...
cUtil$1.getTerrainHeight = function (position, viewer) {
  if (!viewer) {
...
cUtil$1.get3dtilesHeight = function (position, viewer) {
  if (!viewer) {
...
cUtil$1.computeAreaOfTriangle = function (pos1, pos2, pos3) {
  if (!pos1 || !pos2 || !pos3) {
    console.log("传入坐标有误！");
    return 0;
  }
...
cUtil$1.getSlopePosition = function (center, radius, angle, viewer) {
  if (!viewer) {
  ...
  var positions = cUtil$1.getCirclePointsByRadius({
    center: center,
    radius: radius || 10,
    angle: angle || 10
  }, viewer);
  var minH = Number.MAX_VALUE;
  var centerH = cUtil$1.getTerrainHeight(center.clone(), viewer);
  if (!centerH) return;
  var step = -1;

  for (var i = 0; i < positions.length; i++) {
    var h = cUtil$1.getTerrainHeight(positions[i], viewer);
...
var Prompt$1 = /*#__PURE__*/function () {
  ...
  function Prompt(viewer, opt) {
    ...
    this.promptDiv.className = "easy3d-prompt ".concat(this.opt.className);
...
var BasePlot = /*#__PURE__*/function () {
   ...
   */
  function BasePlot(viewer, style) {
  ...
  _createClass(BasePlot, [{
    ...
    key: "setClientScale",
    value: function setClientScale(scale) {
      scale = scale || 1;
      this.clientScale = scale;
    }
  }, {
    key: "transpx",
    value: function transpx(px) {
      return {
    ...
    */
    ...
    value: function getLnglats() {
      return cUtil$1.cartesiansToLnglats(this.positions, this.viewer);
    }
    /**
     * 设置自定义属性
  ...
  _createClass(CreateCircle, [{
    ...
    */
    ...
    value: function getPositions(isWgs84) {
      ...
      if (isWgs84) {
        positions = cUtil$1.cartesiansToLnglats([this.position, this.floatPosition], this.viewer);
      } else {
...
var CreateRectangle = /*#__PURE__*/function (_BasePlot) {
  ...
  _createClass(CreateRectangle, [{
    ...
    value: function getPositions(isWgs84) {
      ...
      if (isWgs84) {
        positions = cUtil.cartesiansToLnglats([this.leftup, this.rightdown], this.viewer);
      } else {
...
Cesium$1.Material._materialCache.addMaterial('WallMaterial', {
  fabric: {
    type: 'WallMaterial',
    uniforms: {
...
Cesium$1.Material._materialCache.addMaterial("WaveMaterial", {
  fabric: {
    type: "WaveMaterial",
    uniforms: {
      color: new Cesium$1.Color(1, 0, 0, 1.0),
...
Cesium$1.Material._materialCache.addMaterial("ScanMaterial", {
  fabric: {
    type: "",
    uniforms: {
      color: new Cesium$1.Color(1, 0, 0, 1.0),
...
Cesium$1.Material._materialCache.addMaterial('cylinderMaterial', {
  fabric: {
    type: Cesium$1.Material.CircleFadeMaterialType,
    uniforms: {
      color: new Cesium$1.Color(1, 0, 0, 1.0),
      time: 1,
      bottom: true
...
var animate = {
  FlowLineMaterial: FlowLineMaterial,
...
var CreatePolyline = /*#__PURE__*/function (_BasePlot) {
  ...
  function CreatePolyline(viewer, style) {
    var _this;
    ...
    */
    ...
    _this.maxPointNum = style.maxPointNum || Number.MAX_VALUE; // 最多点数
  ...
  _createClass(CreatePolyline, [{
    ...
    value: function getMaterial(animateType, style) {
      ...
      if (animateType == "flowline") {
        ...
        material = new animate.FlyLineMaterial({
          //动画线材质
          color: color,
  ...
  _createClass(DrawTool, [{
    ...
    */
    ...
    value: function createDelteDom(px, objId) {
      ...
      var clsBtn = window.document.getElementById("easy3d-plot-delete");
      if (!clsBtn) return;
      var that = this;
      clsBtn.addEventListener("click", function (e) {
...
var draw = {
  drawTool: undefined,
  viewer: undefined,
  start: function start(opt, viewer) {
    var _styleList$filter$;

    if (!this.drawTool) {
      this.viewer = viewer || this.viewer || window.viewer;
      this.drawTool = new DrawTool(this.viewer, {
        canEdit: false
      });
    }
    /* if(opt.success){
        this.drawTool.on("endEdit",opt.success);
    } */
    ...
    return this.drawTool.start(opt);
  },
  "delete": function _delete(entObj) {
    if (!entObj || !this.drawTool) return;
    this.drawTool.removeOne(entObj);
  },
  clear: function clear() {
    this.removeAll();
  },
  remove: function remove(entObj) {
    this["delete"](entObj);
  },
...
var Heatmap = /*#__PURE__*/function () {
  function Heatmap(viewer, opt) {
    ...
    this.createDom();
    var config = {
      container: document.getElementById("easy3d-heatmap-".concat(this.id)),
  ...
  _createClass(Heatmap, [{
    ...
    value: function destory() {
      var dom = document.getElementById("easy3d-heatmap-".concat(this.id));
      if (dom) dom.remove();
    ...
    value: function getBound(positions) {
      ...
      var lnglats = cUtil$1.cartesiansToLnglats(rect, this.viewer);
  ...
  _createClass(GeojsonLayer, [{
    ...
    value: function load(fun) {
      ...
      dataSource.then(function (ds) {
        ...
        for (var i = 0; i < that._layer.entities.values.length; i++) {
          var ent = that._layer.entities.values[i];
          ...
          var color = that.getColorByProperty(style.color, properties);
          color = Cesium.Color.fromCssColorString(color);
          style.color = color.withAlpha(style.colorAlpha || 1);

          if (style.outline) {
            var outlineColor = that.getColorByProperty(style.outlineColor, properties);
...
var TilesetEdit = /*#__PURE__*/function () {
  function TilesetEdit(viewer, opt) {
    ...
    var centerMtx = Cesium.Transforms.eastNorthUpToFixedFrame(this.initBoundingSphere.center.clone());
    this.centerMtx_inverse = Cesium.Matrix4.inverse(centerMtx.clone(), new Cesium.Matrix4());
    this.newCenter = Cesium.Matrix4.multiplyByPoint(this.centerMtx_inverse.clone(), this.initBoundingSphere.center.clone(), new Cesium.Cartesian3());
  } // 计算平移矩阵
  ...
  _createClass(TilesetEdit, [{
    ...
    value: function setPosition(position) {
      ...
      var nowMovelMtx = Cesium.Matrix4.fromTranslation(translation.clone(), new Cesium.Matrix4());
    ...
    value: function setScale(scale) {
      var translation = new Cesium.Cartesian3(1, 1, 1);
      ...
      var nowScaleMtx = Cesium.Matrix4.fromScale(translation.clone(), new Cesium.Matrix4());
    ...
    key: "setScaleX",
    value: function setScaleX(scale) {
      var translation = new Cesium.Cartesian3(scale, 1, 1);
      var nowScaleMtx = Cesium.Matrix4.fromScale(translation.clone(), new Cesium.Matrix4());
    ...
    key: "setScaleY",
    value: function setScaleY(scale) {
      var translation = new Cesium.Cartesian3(1, scale, 1);
      var nowScaleMtx = Cesium.Matrix4.fromScale(translation.clone(), new Cesium.Matrix4());
    ...
    key: "setScaleZ",
    value: function setScaleZ(scale) {
      var translation = new Cesium.Cartesian3(1, 1, scale);
      var nowScaleMtx = Cesium.Matrix4.fromScale(translation.clone(), new Cesium.Matrix4());
    ...
    value: function setRotateX(angle) {
      var mtx = Cesium.Matrix3.fromRotationX(Cesium.Math.toRadians(angle));
      var nowRotationMtx_X = Cesium.Matrix4.fromRotation(mtx, new Cesium.Matrix4());
    ...
    value: function setRotateY(angle) {
      var mtx = Cesium.Matrix3.fromRotationY(Cesium.Math.toRadians(angle));
      var nowRotationMtx_Y = Cesium.Matrix4.fromRotation(mtx, new Cesium.Matrix4());
    ...
    value: function setRotateZ(angle) {
      var mtx = Cesium.Matrix3.fromRotationZ(Cesium.Math.toRadians(angle));
      var nowRotationMtx_Z = Cesium.Matrix4.fromRotation(mtx, new Cesium.Matrix4());
    ...
    key: "revertTransform",
    value: function revertTransform(mtx4) {
      var inverse = Cesium.Matrix4.inverse(mtx4.clone(), new Cesium.Matrix4());
...
var TilesetLayer = /*#__PURE__*/function () {
  ...
  _createClass(TilesetLayer, [{
    ...
    value: function load(fun) {
      ...
      var tilesetAttr = Object.assign(defaultVal, this.opt);
      var test = this.viewer.scene.primitives.add(new Cesium.Cesium3DTileset(tilesetAttr));
      test.readyPromise.then(function (tileset) {
        if (!that.tilesetEdit) that.tilesetEdit = new TilesetEdit(that.viewer, {
          tileset: tileset
        });
        that._layer = tileset;
  ...
  _createClass(MeasureLnglat, [{
    ...
    value: function start(callback) {
      ...
      this.handler.setInputAction(function (evt) {
        ...
        var lnglat = cUtil$1.cartesianToLnglat(cartesian, that.viewer);
...
var GaodeRoute = /*#__PURE__*/function () {
  ...
  _createClass(GaodeRoute, [{
    ...
    key: "queryDriving",
    value: function queryDriving(options, success) {
      options = options || {};
      ...
      var avoidpolygons = undefined; // 避让区域
    ...
    key: "queryUndriving",
    value: function queryUndriving(url, options, success) {
      options = options || {};
...
var viewControl = {
  ...
  isReset: false,
  viewOpt: {},
  activate: function activate(viewer, opt) {
    this.viewer = viewer;
    this.viewOpt = opt || {};
    this.viewOpt.duration = this.viewOpt.duration || 3; // 默认给个初始视角

    this.viewOpt.initView = this.viewOpt.initView || cUtil$1.getCameraView(this.viewer);
  ...
  cameraMoveStartHandler: function cameraMoveStartHandler() {
    console.log("cameraMoveStartHandler");
  },
  cameraMoveEndHandler: function cameraMoveEndHandler() {
    console.log("cameraMoveEndHandler");
    if (this.isReset) return;
    var cameraV = cUtil$1.getCameraView(this.viewer);
  ...
  flyTo: function flyTo(view) {
    ...
    cUtil$1.setCameraView(view, this.viewer);
  }
};
  ...
  _createClass(Roam, [{
    ...
    */
    ...
    value: function reversePositions(positions) {
      ...
      if (positions[0] instanceof Cesium.Cartesian3) {
        return cUtil$1.cartesiansToLnglats(positions, this.viewer);
      } else {
        return positions;
      }
    } // 实时计算相机视角
...
var ZoomTool = /*#__PURE__*/function () {
  ...
  _createClass(ZoomTool, [{
    ...
    value: function computeLength() {
      ...
      var cdir = Cesium.Cartesian3.negate(this.position.clone(), new Cesium.Cartesian3());
      Cesium.Cartesian3.normalize(cdir.clone(), cdir);
      var ray = new Cesium.Ray(this.position.clone(), cdir.clone());
...
var ParticleSystem = function ParticleSystem(viewer, opt) {
  this.viewer = viewer;
  this.opt = opt || {};
...
ParticleSystem.prototype = {
  ...
  start: function start() {
    this.viewer.clock.shouldAnimate = true; // 设置部分默认值
    ...
    if (!this.particleSystem) {
      ...
      this.particleSystem = this.viewer.scene.primitives.add(new Cesium.ParticleSystem({
        ...
        startColor: this.opt.startColor || Cesium.Color.RED.withAlpha(0.7),
        // 开始颜色
        endColor: this.opt.endColor || Cesium.Color.YELLOW.withAlpha(1),
        // 结束颜色
        startScale: particleSystemDefault.startScale,
        // 开始时比例
        endScale: particleSystemDefault.endScale,
        // 结束时比例
        // particleLife: this.opt.particleLife || 2, // 粒子存在的时间
        minimumParticleLife: particleSystemDefault.minimumParticleLife,
        // 粒子生命周期的最小值
        maximumParticleLife: particleSystemDefault.maximumParticleLife,
        // 粒子生命周期的最大值
        ...
        minimumImageSize: new Cesium.Cartesian2(particleSystemDefault.particleSize, particleSystemDefault.particleSize),
        // 图片尺寸最小值
        maximumImageSize: new Cesium.Cartesian2(particleSystemDefault.particleSize, particleSystemDefault.particleSize),
        // 图片尺寸最大值
        // sizeInMeters: this.opt.sizeInMeters || false, // 设置粒子的尺寸单位 默认为像素单位
        loop: this.opt.loop || true,
        // 是否循环播放
        emissionRate: particleSystemDefault.emissionRate,
        // 发射速率
        bursts: bursts,
        lifetime: this.allTime || 16.0,
        // 粒子系统生命周期
        ...
        modelMatrix: Cesium.Transforms.eastNorthUpToFixedFrame(this.position),
        // 粒子效果位置矩阵 
        updateCallback: function updateCallback(p, dt) {
          // 更新粒子回调函数
          var position = p.position;
          Cesium.Cartesian3.normalize(position, gravityScratch);
          Cesium.Cartesian3.multiplyByScalar(gravityScratch, particleSystemDefault.gravity * dt, gravityScratch);
 ...
 * @class
 */
var LimitHeght = /*#__PURE__*/function () {
  /**
   * @param {Cesium.Viewer} viewer 地图viewer对象 
   * @param {Object} opt 基础参数
   ...
   */
  function LimitHeght(viewer, opt) {
    _classCallCheck(this, LimitHeght);
...
var Flat = /*#__PURE__*/function () {
  ...
  _createClass(Flat, [{
    key: "addRegion",
    value: function addRegion(positions, height) {
      this.regionPositions.push(positions);
      var rgOptions = [];

      for (var i = 0; i < this.regionPositions.length; i++) {
        var item = this.regionPositions[i]; // 将面的世界坐标转为局部坐标

        var localCoor = this.cartesiansToLocal(item);
        rgOptions.push(localCoor);
      }

      var vtx1 = this.shaderSource_isPointInPolygon_set(rgOptions);
      var vtx2 = this.shaderSource_isPointInPolygon_vsOutput_set(rgOptions);
      this.updateShader(vtx1, vtx2);
    }
  }, {
    key: "destroy",
    value: function destroy() {
    ...
    */
    ...
    key: "shaderSource_isPointInPolygon_n",
    value:
    /**
     * Shader字符串，生成一个函数，判断点是否在n个元素的数组的多边形里
     */
    function shaderSource_isPointInPolygon_n(n) {
      return "\n        // ".concat(n, "\u4E2A\u5143\u7D20\u7684\u6570\u7EC4\n        vec2 points_").concat(n, "[").concat(n, "];\n        bool isPointInPolygon_").concat(n, "(vec2 point){\n          int nCross = 0; // \u4EA4\u70B9\u6570\n          const int n = ").concat(n, "; \n          for(int i = 0; i < n; i++){\n            vec2 p1 = points_").concat(n, "[i];\n            vec2 p2 = points_").concat(n, "[int(mod(float(i+1),float(n)))];\n            if(p1[1] == p2[1]){\n              continue;\n            }\n            if(point[1] < min(p1[1], p2[1])){\n              continue;\n            }\n            if(point[1] >= max(p1[1], p2[1])){\n              continue;\n            }\n            float x = p1[0] + ((point[1] - p1[1]) * (p2[0] - p1[0])) / (p2[1] - p1[1]);\n            if(x > point[0]){\n              nCross++;\n            }\n          }\n          return int(mod(float(nCross), float(2))) == 1;\n        }");
    }
  }, {
    key: "shaderSource_isPointInPolygon_set",
    value: function shaderSource_isPointInPolygon_set(polygons) {
      var _this = this;
      ...
      var str = "";
      uniqueArray.forEach(function (len) {
        str += _this.shaderSource_isPointInPolygon_n(len) + "\n";
      });
      return str;
    }
  }, {
    key: "shaderSource_isPointInPolygon_vsOutput_set",
    value: function shaderSource_isPointInPolygon_vsOutput_set(coorsArr) {
      var str = "";

      for (var i = 0; i < coorsArr.length; i++) {
        str += this.shaderSource_isPointInPolygon_vsOutput(coorsArr[i]) + "\n";
      }

      return str;
    }
  }, {
    key: "shaderSource_isPointInPolygon_vsOutput",
    value: function shaderSource_isPointInPolygon_vsOutput(coors) {
      var n = coors.length;
      var str = "";
      coors.forEach(function (coordinate, index) {
        str += "points_".concat(n, "[").concat(index, "] = vec2(").concat(coordinate[0], ", ").concat(coordinate[1], ");\n");
      });
...
var RightTool = /*#__PURE__*/function () {
  function RightTool(viewer, opt) {
    ...
    $__default(this.mapContainer).append(toolHtml); // 点击其它地方 关闭面板

    $__default(document).off("click").on("click", function () {
      $__default(".easy3d-right-tool").hide();
    });
    $__default(".easy3d-right-tool").click(function (event) {
  ...
  _createClass(RightTool, [{
    ...
    value: function crateLnglatTool() {
      ...
      $__default("#easy3d-right-tool-".concat(this.randomId, " ul")).append(html);
      $__default("#right-tool-lnglat-".concat(this.randomId)).on("click", function () {
        $__default("#easy3d-right-tool-".concat(that.randomId)).hide();
    ...
    value: function createCameraViewTool() {
      ...
      $__default("#easy3d-right-tool-".concat(this.randomId, " ul")).append(html);
      $__default("#right-tool-view-".concat(this.randomId)).on("click", function () {
        $__default("#easy3d-right-tool-".concat(that.randomId)).hide();
    ...
    value: function crateDepthTool() {
      ...
      $__default("#easy3d-right-tool-".concat(this.randomId, " ul")).append(html);
      $__default("#right-tool-depth-".concat(this.randomId)).on("click", function () {
        $__default("#easy3d-right-tool-".concat(that.randomId)).hide();
        var text = $__default(this).text();

        if (text.indexOf("开") != -1) {
          // 表示当前是开启状态
          $__default(this).text("深度检测（关）");
          that.viewer.scene.globe.depthTestAgainstTerrain = true;
        } else {
          $__default(this).text("深度检测（开）");
    ...
    value: function refreshDepthVal() {
      var oldDepth = this.viewer.scene.globe.depthTestAgainstTerrain;
      var depthVal = !oldDepth ? "深度检测（开）" : "深度检测（关）";
      $__default("#right-tool-depth-".concat(this.randomId)).html(depthVal);
    }
  }, {
    key: "bindHandler",
    value: function bindHandler() {
      ...
      this.rightClickHandler.setInputAction(function (evt) {
        ...
        $__default("#easy3d-right-tool-".concat(that.randomId)).css({
    ...
    value: function destroy() {
      ...
      $__default("easy3d-right-tool-".concat(this.randomId)).remove();
    } // 构建结果面板
    ...
    value: function crerateResultHtml(title, result) {
      var that = this;
      $__default("#easy3d-right-content-".concat(this.randomId)).remove();
      this.createShadow();
      ...
      $__default("body").append(html); // 点击关闭

      $__default("#right-content-close-".concat(this.randomId)).off("click").on("click", function () {
        $__default(this).parent().remove();
        $__default("#easy3d-right-tool-shadow-".concat(that.randomId)).remove();
      });
    } //  创建遮罩
    ...
    value: function createShadow() {
      $__default("#easy3d-right-tool-shadow-".concat(this.randomId)).remove();
      var html = "\n            <div class=\"easy3d-right-tool-shadow\" id=\"easy3d-right-tool-shadow-".concat(this.randomId, "\"></div>\n        ");
      $__default("body").append(html);
      $__default("#easy3d-right-tool-shadow-".concat(this.randomId)).show();
    } // 自主创建

  }, {
    key: "append",
    value: function append(opt) {
      var id = opt.id || new Date().getTime();
      var html = "\n            <li id=\"right-tool-".concat(id, "\">\n                ").concat(opt.content, "\n            </li>\n        ");
      $__default("#easy3d-right-tool-".concat(this.randomId, " ul")).append(html);
      $__default("#right-tool-".concat(id)).on("click", function () {
        if (opt.click) opt.click($__default(this));
      });
    }
  }]);
...
var knockout = Cesium$2.knockout;
...
var knockout$1 = Cesium$3.knockout,
    SvgPathBindingHandler = Cesium$3.SvgPathBindingHandler;
...
var knockout$2 = Cesium$4.knockout,
    getElement = Cesium$4.getElement;
...
var defined = Cesium$5.defined,
    DeveloperError = Cesium$5.DeveloperError,
    EllipsoidGeodesic = Cesium$5.EllipsoidGeodesic,
    Cartesian2 = Cesium$5.Cartesian2,
    getTimestamp = Cesium$5.getTimestamp,
    EventHelper = Cesium$5.EventHelper,
    knockout$3 = Cesium$5.knockout;
...
DistanceLegendViewModel.prototype.show = function (container) {
  ...
  if (this.enableDistanceLegend) {
    testing = '<div class="distance-legend" id="easy3d-distance-legend" data-bind="visible: distanceLabel && barWidth">' + '<div class="distance-legend-label" data-bind="text: distanceLabel"></div>' + '<div class="distance-legend-scale-bar" data-bind="style: { width: barWidth + \'px\', left: ((125 - barWidth) / 2) + \'px\' }"></div>' + '</div>';
  } else {
...
var defined$1 = Cesium$6.defined,
    DeveloperError$1 = Cesium$6.DeveloperError,
    knockout$4 = Cesium$6.knockout;
...
var defined$2 = Cesium$7.defined,
    Camera = Cesium$7.Camera,
    Rectangle = Cesium$7.Rectangle,
    Cartographic = Cesium$7.Cartographic,
    Math$1 = Cesium$7.Math;
/**
...
ResetViewNavigationControl.prototype.setCameraView = function (obj, mapViewer) {
  ...
  if (!obj) return;
  var position = obj.destination || Cesium$7.Cartesian3.fromDegrees(obj.x, obj.y, obj.z); // 兼容cartesian3和xyz

  viewer.camera.flyTo({
    destination: position,
    orientation: {
      heading: Cesium$7.Math.toRadians(obj.heading || 0),
      pitch: Cesium$7.Math.toRadians(obj.pitch || 0),
      roll: Cesium$7.Math.toRadians(obj.roll || 0)
    },
    duration: obj.duration === undefined ? 3 : obj.duration,
    complete: obj.complete
  });
...
var defined$3 = Cesium$8.defined,
    Ray = Cesium$8.Ray,
    Cartesian3 = Cesium$8.Cartesian3,
    Cartographic$1 = Cesium$8.Cartographic,
    ReferenceFrame = Cesium$8.ReferenceFrame,
    SceneMode = Cesium$8.SceneMode;
var Utils = {};
...
var defined$4 = Cesium$9.defined,
    Ray$1 = Cesium$9.Ray,
    IntersectionTests = Cesium$9.IntersectionTests,
    Cartesian3$1 = Cesium$9.Cartesian3,
    SceneMode$1 = Cesium$9.SceneMode;
/**
 * The model for a zoom in control in the navigation control tool bar
 *
...
var Cesium$a = require('cesium/Cesium.js');

var defined$5 = Cesium$a.defined,
    getTimestamp$1 = Cesium$a.getTimestamp,
    EventHelper$1 = Cesium$a.EventHelper,
    Transforms = Cesium$a.Transforms,
    SceneMode$2 = Cesium$a.SceneMode,
    Cartesian2$1 = Cesium$a.Cartesian2,
    Cartesian3$2 = Cesium$a.Cartesian3,
    Matrix4 = Cesium$a.Matrix4,
    BoundingSphere = Cesium$a.BoundingSphere,
    HeadingPitchRange = Cesium$a.HeadingPitchRange,
    knockout$5 = Cesium$a.knockout;
var cesiumMath = Cesium$a.Math;
...
/* eslint-disable no-unused-vars */
var Cesium$b = require('cesium/Cesium.js');

var defined$6 = Cesium$b.defined,
    Event = Cesium$b.Event,
    knockout$6 = Cesium$b.knockout,
    DeveloperError$2 = Cesium$b.DeveloperError;
...
var MapViewer = /*#__PURE__*/function () {
  ...
  function MapViewer(domId, opt) {
    ...
    */
    ...
    this.viewer.scene.debugShowFramesPerSecond = this.opt.map.debugShowFramesPerSecond; // 亮度设置
  ...
  _createClass(MapViewer, [{
    ...
    */
    ...
    value: function openLnglatNavigation() {
      if (!this.lnglatNavigation) this.lnglatNavigation = new LatlngNavigation(this._viewer);
    }
    /**
    * 关闭底部坐标提示
    */
    ...
    value: function openWorldAnimate() {
      ...
      this.setRotate({
        ...
        if (that.opt.map.cameraView) {
          ...
          cUtil$1.setCameraView(that.opt.map.cameraView, that.viewer);
        }
      });
    }
  }, {
...
var greenimg = "green.png";

var blueimg = "blue.png";

var yellowimg = "yellow.png";
...
var Cluster = /*#__PURE__*/function () {
  ...
  _createClass(Cluster, [{
    ...
    value: function getClusterImg(length, fun) {
      ...
      for (var i = 0; i < this.opt.conditions.length - 1; i++) {
        if (length <= this.opt.conditions[0].number) {
          ...
          var nowitem = this.opt.conditions[i];
          var nextitem = this.opt.conditions[i + 1];
      ...
      canvas.width = 48;
      canvas.height = 48; // 获得canva上下文环境

      var ctx = canvas.getContext('2d');
      ctx.fillStyle = "rgb(255,255,255)"; // 设置文字属性

      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      Cesium.Resource.fetchImage(img).then(function (iconDom) {
        ctx.drawImage(iconDom, 0, 0, canvas.width, canvas.height); // 设置背景

        ctx.font = "14px Arial"; // 设置文字
...
var easy3d_export = {
  ...
  CreateGltfModel: CreateGltfModel,
